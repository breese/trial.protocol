...
Writer is an incremental generator that outputs {cpp} data types in a JSON format.
The output is generated piece by piece as {cpp} data types are inserted.
...
|`size_type literal([.blue]#const# view_type&)` |Write a literal value directly into the JSON output without formatting it. Returns the number of characters written. Returns zero if an error occurred.
...
into the output{empty}footnote:[See `example/json/pretty_printer` for an example of how
...
Boolean values are output via `writer::value([blue]#bool#)`.
