...
on it. If the JSON input should end up in your own {cpp} data structures, then the
...
The ability of xref:json:reader.adoc[json::reader] to read the input one
...
described in the Gang-of-Four book, instead of {cpp} iterators.

== Numbers

JSON numbers are arithmetic - there is no distinction between integer and
floating-point numbers.
{cpp} does make that distinction, so xref:json:reader.adoc[json::reader]
...
as either a {cpp} integer or floating-point number.
This means that integer numbers that are too big to fit into a {cpp} integer type
