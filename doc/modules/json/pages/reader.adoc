...
JSON input into a sequence of {cpp} tokens.
...
the current xref:json:token.adoc[token].
...
|`[blue]#const# view_type& literal()` |Returns a view of the raw input of the current value.
...
requested by `reader::value<[blue]##bool##>()`.
...
While JSON does not distinguish between integer and floating-point numbers, {cpp}
...
can be thought of as a `std::map` in {cpp}.
