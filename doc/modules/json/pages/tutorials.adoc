...
This tutorial shows how the xref:json:oarchive.adoc[JSON output archive] and the xref:json:iarchive.adoc[JSON input archive] can be used to serialize {cpp} data into JSON and deserialize JSON into {cpp} data in just a few lines of code.
...
archive. We can choose between several xref:core:adapter.adoc[buffer types].
...
We also need to include xref:json:oarchive.adoc[json::oarchive] together
...
We can deserialize the buffer again with the xref:json:iarchive.adoc[JSON input archive].
...
There is also built-in support for serialization of certain standard {cpp} containers,
...
from JSON into {cpp} types.

We are going to start with incremental generation to create JSON formatted
output using the xref:json:writer.adoc[json::writer].
Afterwards we will use the xref:json:reader.adoc[json::reader] to parse
JSON formatted input.

=== Generating fundamental types

The xref:json:writer.adoc[json::writer] is used to incrementally generate
...
We now turn our attention to incremental parsing. xref:json:reader.adoc[json::reader] is a pull parser that lazily parses a single token in the input.
...
We will use xref:json:reader.adoc[json::reader] to build the push parser,
...
a template parameter.{empty}footnote:[We could also have used a polymorphic interface
